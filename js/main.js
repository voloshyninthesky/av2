...
import './core/errlog.js?v=20260905-06';
import './core/telegram.js?v=20260905-06';
import { session, easeInOut } from './core/session.js?v=20260905-06';
import { mascotMove, dance } from './mascot/state.js?v=20260905-06';
import { play } from './play/state.js?v=20260905-06';
import { raycaster, pointer, stageWalkPlane } from './view/pick.js?v=20260905-06';
import './shell/qa-hooks.js?v=20260905-06';
import {
  chipFor,
  queuePriceChip,
  ...
  initVibe,
} from './play/vibe.js?v=20260905-06';
import {
  loop,
  updateLoopProgress,
  initLoopPedal,
} from './play/loop.js?v=20260905-06';
import {
  currentGuitarChordName,
  allGuitarPitches,
  initGuitarPlay,
} from './play/guitar.js?v=20260905-06';
import {
  captureHeldVocalIntoLoop,
  finishHeldLoopCapture,
  initPads,
} from './play/pads.js?v=20260905-06';
import {
  showRibbon,
  hideRibbon,
  stopRibbonVoice,
  initRibbon,
} from './play/ribbon.js?v=20260905-06';
import {
  showChordWheel,
  hideChordWheel,
  ...
  initChordWheel,
} from './play/chord-wheel.js?v=20260905-06';
import {
  showGrooveWheel,
  hideGrooveWheel,
  stopGroove,
  updateGroovePlayhead,
  grooveBarSeconds,
  grooveBeatSeconds,
  grooveDownbeatAt,
  initGroove,
} from './play/groove.js?v=20260905-06';
import {
  beginHeldPianoNote,
  ...
  initPianoNotes,
} from './play/piano-notes.js?v=20260905-06';
import {
  composer,
  bloomPass,
  initPostprocessing,
  ...
  qualityWarmup,
  initPostfx,
} from './shell/postfx.js?v=20260905-06';
import {
  FLY_DUR,
  shouldSkipIntro,
  startOnboard,
  ...
  initIntro,
} from './shell/intro.js?v=20260905-06';
import {
  closeSoundMixer,
  releaseKeyboardVocal,
  beginKeyboardVocal,
  initMixer,
} from './play/mixer.js?v=20260905-06';
import {
  giftCam,
  queueGiftRefit,
  updateGiftReveal,
  ...
  giftApproachFraming,
  initMascotGift,
} from './mascot/reveal.js?v=20260905-06';
import {
  params,
  ...
  stageAmbience,
  stageLightLevel,
} from './core/quality.js?v=20260905-06';
import { onCameraModeChange } from './core/camera-mode.js?v=20260905-06';
import {
  canvas,
  renderer,
  scene,
  ...
  CAM_END,
  TARGET,
} from './view/rig.js?v=20260905-06';
import {
  ui,
  audio,
  stage,
  ...
  instruments,
  interactables,
} from './core/studio.js?v=20260905-06';
import { spotHeads, applyLowMobileSceneBudget } from './scene/lighting.js?v=20260905-06';
import {
  instrumentView,
} from './view/instrument-presets.js?v=20260905-06';
import {
  updateInstrumentViewCamera,
  initInstrumentView,
} from './view/instrument-view.js?v=20260905-06';
import {
  setDancing,
} from './mascot/pose.js?v=20260905-06';
import {
  PHOTO_SLIDES_ENABLED,
  loadSlideTextures,
  updateSlideshow,
  updateSlideshowNavLayout,
} from './scene/slideshow.js?v=20260905-06';
import { hitPulse, bumpHitPulse } from './scene/effects.js?v=20260905-06';
import {
  setGlow,
  isInstrumentCloseUp,
  onPointerMove,
  ...
  canKeyboardJamPlay,
  syncOrbitZoom,
} from './view/pointer.js?v=20260905-06';
import {
  syncViewportMeta,
  ...
  initViewport,
} from './view/viewport.js?v=20260905-06';
import {
  ...
  initMobileControls,
} from './view/mobile-controls.js?v=20260905-06';
import { updateMascot } from './mascot/update.js?v=20260905-06';
import { initSigns, revealSigns } from './shell/signs.js?v=20260905-06';
import { updateSigns } from './scene/signs.js?v=20260905-06';
...
initMobileControls({ playNearestInstrument, hideRibbon, hideChordWheel, hideGrooveWheel });
...
initLoopPedal({
  ...
  finishHeldPianoLoopCaptures,
  finalizeHeldPianoLoopCapture,
  // The loop's bar comes from the groove when one is running, so a take
  // quantises to whole bars instead of to a grid of eighths of a second —
  // and the beat drives the pedal's count-in to the next downbeat.
  grooveBarSeconds,
  grooveBeatSeconds,
  grooveDownbeatAt,
});
initGuitarPlay({ isGuitarPlayFocus, markHeldTouchGuitarChordUsed });
initPads({ isLiveStageZoomLocked });
// The ribbon is the mic's whole surface, so it needs the same three things the
// wheels do: permission to wake audio, which instrument is listening, and the
// keyboard voice to stand down when a finger takes over.
initRibbon({
  activateAudioForSound,
  canPlayInstrument,
  releaseKeyboardVocal,
...
// The groove wheel needs to know which instrument is listening, and it is the
// one surface that must never wake audio on its own — only its sound pill does,
// through the hook below.
initGroove({
  activateAudioForSound,
  canPlayInstrument,
  canKeyboardJamPlay,
  loopHasContent: () => loop.state !== 'empty',
  loopIsRecording: () => loop.state === 'recording' || loop.state === 'overdubbing',
  toast: (message, ms) => ui.toast(message, ms),
});
initPianoNotes({
  activateAudioForSound,
  canPlayInstrument,
...
function setInstrumentViewPhase(phase, kind = instrumentView.kind) {
  ...
  // One dock, three surfaces, exactly one ever shown — asserted in
  // window.__ribbonDebug().docked rather than trusted. Each instrument gets
  // the one that fits the gap it has: chords for guitar and piano, time for
  // drums, and the voice's own continuous pitch and vowel for the mic.
  if (phase === 'focused' && kind === 'mic') {
    hideChordWheel();
    hideGrooveWheel();
    showRibbon();
    document.documentElement.classList.remove('guitar-focused', 'guitar-fretting');
  } else if (phase === 'focused' && kind === 'drums') {
    hideRibbon();
    hideChordWheel();
    showGrooveWheel();
    document.documentElement.classList.remove('guitar-focused', 'guitar-fretting');
...
function animate(frameTime = performance.now()) {
  ...
  updateGiftReveal(dt);
  updateMascot(dt);
  // Deliberately outside updateMascot: that returns early while the reveal owns
  // the mascot, and the companion must keep living while the card is up.
  mascotCompanion.update(t, dt, instrumentView.phase !== 'idle');
  updateMobilePlayAvailability();
