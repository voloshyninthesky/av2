...
- `av2.groove.v1` holds the groove wheel's choice as `{ groove, bpm }` — `groove` an index `0–11`, `bpm` `60–176` (§ The groove wheel). Each field falls back on its own, to ПУЛЬС at 92. The stored tempo is the visitor's last one and may differ from the stored genre's own — the next wedge tapped puts them back in step. **The sound state is deliberately absent**: it resets to silence on every load, because a remembered unmute would start drums at a returning visitor who has not asked for any.
