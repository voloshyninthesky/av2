...
- A tempo change holds the bar's position, stopped or playing, so the beat does not jump under a groove that is only paused. That holds for the tempo a genre brings with it as well as for the stepper.
...
- **A switch repairs the route it plays through.** Every tap on a wedge is a trusted gesture, so every tap may rebuild an audio context that was marked for recovery — not only the tap that starts from stopped. The bar's epoch is a reading of *one* context's clock and is re-pinned whenever that context is replaced, since a clock that restarted at zero puts the old epoch in its future and schedules every hit for a bar that never comes. Without both halves a groove that was playing across a blur or a tab away goes silent and stays silent through any number of switches, until one happens to stop and start it. `__grooveDebug().epochStale` must never be true.
...
- **The genre carries its tempo.** Every groove declares the speed its style is played at — ДНБ `172`, БАЛАДА `72`, ХАУС `124` — and choosing the wedge moves the tempo there in the same tap. A genre is its tempo as much as its pattern: the drum-and-bass break at `76` is the pattern without the style, and nobody reaches `172` by tapping a stepper twenty times. The stepper still overrides it afterwards. While a **loop** has content the wedge switches the pattern alone: a genre tap is pattern intent that merely carries a tempo default, and browsing genres must not re-time a recorded take as a side effect — only the deliberate stepper does that (§ The groove and the loop pedal).
- **A tempo the visitor set is that genre's tempo from then on.** Stepping the tempo is them saying "this style, at *this* speed", so the wedge brings **their** number back — after a stop and a start, after a trip round the other eleven wedges, and after a reload. The genre's own tempo is a **default**, not an override; without this the stepper would be a control that undoes itself the moment it is used. One remembered tempo per groove, so re-tuning ХАУС leaves ДНБ alone.
- **Tempo is a stepper, not a drag**, `60`–`176` in steps of `4`, and its readout is plain text rather than a button because — unlike the chord wheel's key readout, which doubles as the mode control — it has no second meaning. The ceiling is where it is because ДНБ lives at `172`: a genre the stepper cannot reach would be one the wheel could only play wrong.
...
1. **The groove as a stage-wide backbone**, available under the guitar and piano close-ups too — both roadmaps already ask for a metronome and a backing groove.
...
- **By default (КАМЕРА → Вільна, § Камера) one finger orbits and two fingers pinch-dolly, with the pitch opened up.** Choosing **Не дуже** swaps that for one-finger pan that scouts back to the mascot. The mascot stays followed either way.
...
- **Не дуже** (`follow`) is the original framed camera: the pursuit rig keeps the mascot composed low-centre, a drag temporarily scouts, and the spring recentres on release. Mobile pans on one finger, desktop orbits, pitch stays at **40°–84°**.
...
- `av2.groove.v1` holds the groove wheel's choice as `{ groove, bpm }` — `groove` an index `0–11`, `bpm` `60–176`, and `tempos` a sparse `{ index: bpm }` of the tempos the visitor set by hand, one entry per groove and only for grooves they re-tuned (§ The groove wheel). Each field falls back on its own, to ПУЛЬС at 92; `tempos` is absent in records written before genres carried a tempo, and an entry naming an unknown index or an out-of-range tempo is dropped rather than failing the read. **The sound state is deliberately absent**: it resets to silence on every load, because a remembered unmute would start drums at a returning visitor who has not asked for any.
...
- **A genre arrives at its own speed.** Tapping ДНБ reads `172` and sounds like it; tapping БАЛАДА reads `72`. Stepping the tempo afterwards still moves it, and with a loop recorded the tempo does not move at all — the wedge changes the pattern and nothing else. The bar never jumps: the phase is held across the change, playing or stopped.
- **...and comes back at the visitor's.** Step ХАУС from `124` to `132`, stop it, start it: `132`. Tap round to ДНБ (`172`, untouched) and back: `132` again, and still `132` after a reload. Only the re-tuned groove remembers anything.
