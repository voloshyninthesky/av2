...
**The reveal pose must be a legal orbit state.** The framing distance is clamped to `controls.minDistance`, and closing the card eases `controls.target` off its safe-rect offset onto the character — that offset tips the camera past `controls.maxPolarAngle`. Either one left unhandled makes OrbitControls clamp on its first update and lurch the camera a frame after **ГОТОВО**. Nothing but the gift's own tween may write to the rig while it is running.
